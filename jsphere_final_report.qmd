...
**Single-Category Results.** The finer granularity reduces the overlap among
each sphere compared to our results in @sec-class-results.
@tbl-context-classification shows frontend processing to
be much more popular than the other spheres, accounting for 36% of code.
DOM element generation, UX enhancement, and
extensional features all enjoy similar popularities of around 22%;
considering that 50.9% of code is not silent, we can argue that nearly half of
the functional code does some kind of DOM element generation, or
a similar argument for the other two spheres.
Lower than the vanilla setup, the 38% of code that
is classified into at least one sphere accounts for 3/4 of the non-silent code,
and the 12.4% of code that fall into all sphere accounts for 1/3 of
classified code.
<!-- TODO: WTF is this? -->
...
Script Size Distribution per Sphere per
Subdomain.](data/subdomain_spheres_size_stacked.pdf){width=45%
#fig-size-sphere-stacked}
