...
We automate browsers to crawl and collect API call logs from
...
Hoisting and scoping immediately impact correctness.
Naively, one may group all the top-level statements in
...
Efficacy also requires more sophistication than splitting scripts only on
the top level.
This requirement is due to the extensive use of closure functions in JS, where
...
IIFE.
We preserve blocks with `break`, `continue`, `yield` and `export` to
avoid syntax errors.
...
`eval`'s argument.
Finally, we increase Chromium's stack size to 4 GB and limit the depth of
`eval` nesting to 8, to avoid exceeding the stack size limit.
Some of
