...
acm-metadata:
    ...
    copyright: none
abstract: |
    JavaScript (JS) scripts are popular among websites and have high impact on
    users browser the Web. Why is JS used so extensively?
    What purposes does it serve?
    We begin the first known research to
    classify JS scripts into common functionalities, which we call *spheres*.
    Through web crawling and JS API call analysis,
    ...
    many large scripts perform multiple functionalities, based on which
    we propose a novel approach to
    break down large JS scripts into smaller execution contexts.
    We find that frontend processing remains the core of JS usage, but half of
    the functional code is related to Document Object Model (DOM)
    element generation.
    Additionally, we observe half of the code to not invoke any browser APIs or
    mutate global state, which may suggest much JS code is unnecessary.
...
Additionally, interacting client-side-only information,
including reading webpage `Location`, writing to
search parameters `URLSearchParams`, element text `textContent`, or
...
These DOM APIs include the `createElement`, `createElementNS`,
`createTextNode`, `appendChild`, `insertBefore` and
`CSSStyleDeclaration.setProperty` functions or modifying the values of
`CSSStyleDeclaration` or `style`.
...
set font faces with `FontFaceSet.load`, we consider it a UX enhancement.

**Extensional features.** Currently, we classify scripts that
extract webpage performance data by interacting with the `Performance`,
`PerformanceTiming` or `PerformanceResourceTiming` objects, or
sends tracking data with `Navigator.sendBeacon`, as extensional features.

There are no clear ways to validate our heuristics due to the lack of
ground truth about developer intentions.
One possible approach is to let a human or language model inspect the code to
determine which sphere it belongs to, but JS scripts on
the Web are usually minified and unreadable,
thus manual classification is laborious and unlikely to be more accurate than
our heuristics, which directly rely on the semantics of JS APIs.
...
@dbl-context-classification shows frontend processing continue being much more
...
| DOM Element Generation | 6250.1    | 23.89    |
