...
- **silent**: code that does not invoke any JS APIs, likely never executed.

To infer these spheres, we will analyze JS browser API invocations when
interacting with these websites.
We plan to follow prior practices [@snyder2016browser] when interacting with
each website: we will "monkey test" its root page,
...
classify most JS code through a small number of popular APIs.
We will conduct conduct random sampling to
...
Accompanied with these distributions,
we will report the classification accuracy of
our heuristics during manual validation.
...
<!-- 1. what specific *deliverables* you will have for
    Project B.-->
We will include the following deliverables for Project B:
...
- The resulting code, measurement data, and analysis documentation.
    ...
    The measurement data will include the website domains, page load time and
    bytes, the API traces, the JS files, and the relations among them.
    We will additionally include the process and results of
...
- If the project proposal is approved, a 3-8 page Project C report and
