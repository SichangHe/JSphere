...
The HAR files contain detailed information about the network requests made.
...
Our preliminary analysis of the top 100 subdomains revealed a small set of
APIs dominating JavaScript usage, based on which
we developed our initial sphere classification heuristics and
got reasonable results.

<!-- TODO: Script size distribution -->
...
APIs dominates JavaScript usage on the web.
...
scripts by size into at least one sphere (inverse of "No Sure Sphere"), though
...
<!-- TODO: Better caption. -->
![Cumulative distribution fraction of all scripts and scripts in
each sphere out of the total number of scripts in each category, as
...
those spheres.
In fact, 1296.1MB of scripts (40.60% by size) fall into all sure spheres,
although the count is only 1473 (3.67%).
This suggests a small fraction of large scripts are responsible for
most functionalities, and that the script file is not fine-grained enough to
separate spheres for meaningful purposes.
