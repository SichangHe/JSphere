...
more of the following rough categories, which we call *spheres*:
...
- **Extensional features.** Authentication, tracing, and
...
While perfect classification is impossible because
JS is a Turing-complete language,
browser API calls reveal the effects each script has on the webpage.
...
is interacted, we can approximately infer its purpose.
Thus, we visit and interact with webpages for the top 1000 popular domains to
collect API call logs, then analyze these logs to
classify their JS into spheres, as we detail in @sec-methods.
